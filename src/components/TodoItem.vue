...
<div class="card card--todo flex" v-bind:class="{ 'card--active': isActive(todo.id) }" @click="setActiveTodo">
	...
	<nav class='nav nav_todo flex flex--center' v-if="isActive(todo.id)">
		<div class="col col--3 button button--white button--sm" @click="projectMenu = !projectMenu">
			Move ->
		</div>
		<div class="col col--3 button button--white button--sm" @click="remove">
			Delete
		</div>
	</nav>
    <nav class='nav nav_todo_projects flex' v-if="isActive(todo.id) && projectMenu">
        ...
        <ul class='col col--12'>
            <li v-for="project in projects" class="button button--white button--sm" @click="addToProject(project.id)">
    			{{project.title}}
    		</li>
        </ul>
